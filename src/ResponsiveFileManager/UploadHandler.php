...
class UploadHandler
{
    ...
    public function onUploadEnd($ftp, $res)
    {
        ...
        if ($is_img) {
            ...
            if (session()->exists('RF.thumbnailWidth') && session()->exists('RF.thumbnailHeight')) {
                $newWidth = session('RF.thumbnailWidth');
                $newHeight = session('RF.thumbnailHeight');
            }

            $thumbResult = RFM::createImg($ftp, $targetFile, $targetFileThumb, $newWidth , $newHeight);
